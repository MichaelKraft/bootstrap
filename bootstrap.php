...
//  # preference                                          # key                                                      # value            # sudo?
	...
	Preference::assert('Dock Tilesize',                   'com.apple.dock tilesize',                                 42,                false);
	...
	class Brew
	{
		static function install()
		{
			...
			else
			{
				...
				exec('sudo chown -R $(whoami) /usr/local/var/homebrew');
				exec('sudo chown -R $(whoami) /usr/local/Homebrew/');
				exec('brew update');
				exec('brew upgrade');
			}
		}
